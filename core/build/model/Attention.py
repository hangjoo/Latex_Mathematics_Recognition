...
class AttentionCell(nn.Module):
        ...
        if num_layers == 1:
            ...
            if cell_type == "LSTM":
                self.rnn = nn.ModuleList(
                    [nn.LSTMCell(src_dim + embedding_dim, hidden_dim)] + [nn.LSTMCell(hidden_dim, hidden_dim) for _ in range(num_layers - 1)]
                )
            elif cell_type == "GRU":
                self.rnn = nn.ModuleList(
                    [nn.GRUCell(src_dim + embedding_dim, hidden_dim)] + [nn.GRUCell(hidden_dim, hidden_dim) for _ in range(num_layers - 1)]
                )
            else:
                raise NotImplementedError
    ...
    def forward(self, prev_hidden, src, tgt):  # src: [b, L, c]
        ...
        attention_logit = self.score(torch.tanh(src_features + prev_hidden_proj))  # [b, L, h]  # [b, L, 1]
...
class AttentionDecoder(nn.Module):
    def __init__(
        self, num_classes, src_dim, embedding_dim, hidden_dim, pad_id, st_id, num_layers=1, cell_type="LSTM", checkpoint=None,
    ):
        super(AttentionDecoder, self).__init__()

        self.embedding = nn.Embedding(num_classes + 1, embedding_dim)
        self.attention_cell = AttentionCell(src_dim, hidden_dim, embedding_dim, num_layers, cell_type)
        self.hidden_dim = hidden_dim
    ...
    def forward(self, src, text, is_train=True, teacher_forcing_ratio=1.0, batch_max_length=50):
        """
        input:
        ...
        if self.num_layers == 1:
            ...
            hidden = [
                (torch.FloatTensor(batch_size, self.hidden_dim).fill_(0).to(device), torch.FloatTensor(batch_size, self.hidden_dim).fill_(0).to(device),)
        ...
        if is_train and random.random() < teacher_forcing_ratio:
            ...
            targets = torch.LongTensor(batch_size).fill_(self.st_id).to(device)  # [START] token
...
class Attention_baseline(nn.Module):
    def __init__(self, config, tokenizer):
        ...
        self.decoder = AttentionDecoder(
            ...
            src_dim=config.model.Attention.src_dim,
            embedding_dim=config.model.Attention.embedding_dim,
            ...
            num_layers=config.model.Attention.layer_num,
            cell_type=config.model.Attention.cell_type,
        )
